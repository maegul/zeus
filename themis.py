...
class Themis:
	...
	def _sort(self, use_codes = False, conditions = 9, trials = 10, stim_len = None,
			 ...
			 omit_marker_codes=None, omit_trials = None, omit_conds = None,
			  n_bootstrap=2000,
		...
		spont_method : str
			Method for calculating spontaneous activity
		...
		self.spont_spikes : list
			All spike times collected to calculate the spontaneous firing rate.
			Time recorded in absolute seconds as provided by source data

		self.spont_rate : float
			Rate of spontaneous firing calculated by whatever method user chooses.
		...
		if use_codes:
			...
			if omit_marker_codes is not None:
				assert hasattr(omit_marker_codes, '__iter__') or isinstance(omit_marker_codes, (int,float) ), 'Omit_trials not iterable or number'			
				if isinstance(omit_marker_codes, (float, int)):
					omit_marker_codes = [omit_marker_codes]

				# invert to False when found so that it is omitted
				mask = np.in1d(self.marker_codes, omit_marker_codes, invert=True) 
...
#==============================================================================
	...
	#==============================================================================
		...
		# Taking rate around each marker
		if spont_method == 'peri_marker':
			self.spont_spikes = []

			# Iterate through each marker, aggregating the spikes within the peri_marker_window
			for m in self.markers:

				# Allow for both positive and negative windows (ie, before and after marker)

				window_range = [m, m+peri_marker_window]
				window_range.sort()
				window_mask = (self.spikes >= window_range[0]) & (self.spikes <= window_range[1])

				self.spont_spikes.extend(
						np.extract( 
							window_mask, self.spikes
							)
					)

			# rate is number of spikes divided by the amount of time they were collected from 
			# to get a rate in hertz
